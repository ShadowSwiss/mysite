...
knitr::knit_engines$set(plotly = plotly::renderPlotly)
...
library(readxl)
data1 <- read_excel("~/Documents/Data/Chomage/Indicateur du chomage.xlsx", 
    sheet = "Feuil1")
...
data1 <- data1 %>%
  mutate(across(-Indicators, as.numeric))

data1_long <- data1 %>%
  pivot_longer(cols = -Indicators, names_to = "Date", values_to = "Values") %>%
  mutate(Date = factor(Date, levels = unique(Date), ordered = TRUE))  



plot <- plot_ly(data1_long, x = ~Date, y = ~Values, color = ~Indicators,
                colors = c("#417d95", "#c1dee2", "#89b9c5"),  
                type = 'scatter', mode = 'lines') %>%
  layout(title = "Figure 1: Unemployement rate by SECO, ILO and Jobless rate in percentage of the labor force (2022-Q2 - 2023-Q4)",
         font = list(size = 8),
         yaxis = list(title = "Labor force's percentage"),
         xaxis = list(title = "Quarter"))

plot$x$layout$legend <- list(
  x = 0,
  y = -0.2,
  orientation = "h",
  bgcolor = "rgba(255, 255, 255, 0)",
  font = list(size = 10),
  title = list(text = "Source: FSO, SECO", side = "top", font = list(color = "grey"))
)
...
```{r, echo=FALSE, message=FALSE}
library(readxl)
data2 <- read_excel("~/Documents/Data/Chomage/Indicateur du chomage.xlsx", 
    sheet = "Feuil3")
...
data2_long <- data2 %>%
  pivot_longer(cols = -Country, names_to = "Date", values_to = "Values") %>%
  mutate(Date = factor(Date, levels = unique(Date), ordered = TRUE))  

plot2 <- plot_ly(data2_long, x = ~Date, y = ~Values, color = ~Country,  
                type = 'scatter', mode = 'lines', colors = c("#C6DBEF", "#FF0000", "#AEC7E8", "#9ECAE1", "#6BAED6", "#1F77B4")) %>%
  layout(title = "Figure 2: Jobless rate in the European Union, Germany, France, Austria and Switzerland  (2015-Q1 - 2023-Q4)",
         font = list(size = 8),
         yaxis = list(title = "Total population's percentage (15 to 74 years old)"),
         xaxis = list(title = "Quarter"))

plot2$x$layout$legend <- list(
  x = 0,
  y = -0.2,
  orientation = "h",
  bgcolor = "rgba(255, 255, 255, 0)",
  font = list(size = 10),
  title = list(text = "Source: Eurostat", side = "top", font = list(color = "grey"))
)
